...
export const FetchTrendingAnime = async () => {
  const res = await axios.get(`${BASE_URL}/top/anime?filter=bypopularity`);
  return res.data.data;
};

// ✅ 2. Top Rated / Currently Airing Anime
export const FetchTopRatedAnime = async () => {
  const res = await axios.get(`${BASE_URL}/top/anime?filter=airing`);
  return res.data.data;
};

// ✅ 3. Search Anime by Keyword
export const SearchAnime = async (query) => {
  const res = await axios.get(`${BASE_URL}/anime?q=${encodeURIComponent(query)}&limit=20`);
  return res.data.data;
};
...
export const FetchUpcomingAnime = async () => {
  const res = await axios.get(`${BASE_URL}/top/anime?filter=upcoming`);
  return res.data.data;
};

// ✅ 8. Anime by Genre ID
export const FetchAnimeByGenre = async (genreId) => {
  const res = await axios.get(`${BASE_URL}/anime?genres=${genreId}&limit=20`);
  return res.data.data;
};

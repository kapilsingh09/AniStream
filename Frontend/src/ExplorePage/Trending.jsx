...
const Trending = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 py-12 px-4 md:px-12">
      {/* Header */}
        ...
        {trendingAnime.length === 0
          ...
          : trendingAnime.map((anime, idx) => (
              ...
              >
                {/* Rank Badge */}
                <div
                  className={`absolute top-2 right-2 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1 ${getRankColor(
                    idx
                  )}`}
                >
                      ...
                      {anime.genres.slice(0, 2).map((genre, genreIdx) => (
                        <span
                          ...
                          className={`px-2 py-0.5 rounded-full font-medium border ${
                            genreColors[genre.name] || 'bg-gray-600 text-gray-200 border-gray-400/30'
                          }`}
                        >
                          {genre.name}
                        </span>
      ...
      {/* <div className="text-center mt-12">
        <button className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-purple-500/25">

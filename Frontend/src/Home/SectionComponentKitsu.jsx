...
const SectionComponentKitsu = ({ subtitle = '' , title = "Trending Anime", fetchFunction, className = "" }) => {
  ...
  const handleImageError = (e) => {
    e.target.src = 'https://via.placeholder.com/300x400/1f1f1f/6b7280?text=No+Image';
  };
  ...
  if (loading) {
    return (
      <div className={`h-[60vh] py-3 flex items-center justify-center bg-gray-950 ${className}`}>
  ...
  if (error) {
    return (
      <div className={`h-[60vh] bg-gray-950 py-3 flex items-center justify-center ${className}`}>
        <div className="text-gray-300 text-center">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center gap-2 bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 disabled:from-gray-800 disabled:to-gray-900 text-gray-200 px-6 py-3 rounded-lg font-medium transition-all duration-200 disabled:cursor-not-allowed"
          >
            <RefreshCw 
              size={18} 
  ...
  return (
    <div className={`h-[60vh] bg-gray-950 py-3 ${className} relative text-gray-200`}>
      <div className="absolute left-0 top-0 bottom-0 w-20 bg-gradient-to-r from-gray-950/80 to-transparent z-10 pointer-events-none"></div>
      <div className="absolute right-0 top-0 bottom-0 w-20 bg-gradient-to-l from-gray-950/80 to-transparent z-10 pointer-events-none"></div>

      <div className="flex items-center px-10 mb-2">
        <h1 className="text-gray-200 text-2xl font-bold drop-shadow-lg">{title}</h1>
        <br />
        <div className="flex-1 h-px bg-gray-600/30 ml-4"></div>
        <div className="text-gray-400 text-sm ml-4 drop-shadow">{animeData.length} items</div>
      </div>

        <h3 className= " ml-10 text-gray-400 text-sm  font-bold drop-shadow-lg">{subtitle}</h3>
      <div className="relative px-10">
          ...
          {animeData.map((anime, index) => {
            ...
            return (
              <motion.div
                ...
                className="min-w-[13vw] max-w-[13vw] rounded-xl overflow-hidden text-gray-200 flex flex-col hover:scale-[1.03] transition-transform duration-300 cursor-pointer group"
              >
                <div className="relative h-[40vh] w-full">
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gray-900/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded"></div>
                  
                  {attr?.averageRating && (
                    <motion.div className="absolute top-2 right-2 bg-gray-800 text-gray-200 text-xs px-2 py-1 rounded backdrop-blur-sm">
                      ⭐ {Math.round(attr.averageRating / 10 * 10) / 10}
                    </motion.div>
                  )}
                  
                  {attr?.popularityRank && (
                    <div className="absolute top-2 left-2 bg-gray-700 text-gray-200 text-xs px-3 py-1 rounded backdrop-blur-sm">
                      #{attr.popularityRank}
                    </div>
                  )}
                </div>
                ...
                <div className="py-2 ml-1 text-sm font-medium leading-tight h-[4.5vh]">
                  <div className="line-clamp-2 group-hover:text-gray-300 transition-colors" title={attr?.titles?.en_jp || attr?.titles?.en}>
        ...
        <AnimatePresence>
          {hoveredAnime && (
            <motion.div
              ...
              className="fixed z-[60] w-80 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-950 rounded-xl shadow-2xl border border-gray-700/50 overflow-hidden pointer-events-auto"
              style={{
            ...
            >
              <div className="relative h-40">
                ...
                {/* Auto-hide indicator */}
                <div className="absolute top-2 right-2 bg-gray-700 text-gray-200 px-2 py-1 rounded-full text-xs font-bold">
                  Auto-hide: 2s
                </div>
                
                <div className="absolute bottom-2 left-3 right-3">
                  <h3 className="text-gray-200 font-bold text-lg mb-1 drop-shadow-lg">
                    {hoveredAnime.attributes?.titles?.en_jp || hoveredAnime.attributes?.titles?.en}
                  </h3>
                  <div className="flex items-center gap-2 text-sm">
                    {hoveredAnime.attributes?.averageRating && (
                      <span className="bg-gradient-to-r from-gray-600 to-gray-700 text-gray-200 px-2 py-1 rounded-full text-xs font-bold">
                        ⭐ {Math.round(hoveredAnime.attributes.averageRating / 10 * 10) / 10}
                      </span>
                    )}
                    {hoveredAnime.attributes?.startDate && (
                      <span className="bg-gray-800/60 backdrop-blur-sm text-gray-200 px-2 py-1 rounded-full text-xs">
              ...
              <div className="p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-3">
                  {hoveredAnime.attributes?.episodeCount && (
                    <div>Episodes: <span className="text-gray-300">{hoveredAnime.attributes.episodeCount}</span></div>
                  )}
                  {hoveredAnime.attributes?.status && (
                    <div>Status: <span className="text-gray-300">{hoveredAnime.attributes.status}</span></div>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-2 bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 text-gray-200 text-sm rounded-lg transition-all duration-200 font-medium"
                  >
                    Watch Now
                  </button>
                  <button
                    ...
                    className="flex-1 py-2 bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-500 hover:to-gray-600 text-gray-200 text-sm rounded-lg transition-all duration-200 font-medium flex items-center justify-center gap-1"
                  >
                    <Info size={14} />
                    More Info
                  </button>
        ...
        {canScrollLeft && (
          <motion.button
            onClick={scrollLeft}
            className="absolute top-1/2 -translate-y-1/2 left-12 bg-gray-800/90 hover:bg-gray-700 p-3 rounded-full text-gray-200 shadow-lg transition-all duration-200 hover:scale-110 z-10"
        ...
        {canScrollRight && (
          <motion.button
            onClick={scrollRight}
            className="absolute top-1/2 -translate-y-1/2 right-12 bg-gray-800/90 hover:bg-gray-700 p-3 rounded-full text-gray-200 shadow-lg transition-all duration-200 hover:scale-110 z-10"
